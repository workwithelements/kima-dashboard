...
export default function PacingCard({ pacing, currency = "GBP" }: { pacing: PacingResult; currency?: string }) {
      ...
      <div className="mt-4 grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="mt-0.5 text-lg font-semibold tabular-nums">
            {pacing.budget ? fmtCurrency(pacing.budget, currency) : "Not set"}
          </p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-wider text-neutral-500">Spent to Date</p>
          <p className="mt-0.5 text-lg font-semibold tabular-nums">
            {fmtCurrency(pacing.spentToDate, currency)}
          </p>
        </div>
      </div>
        ...
        <div className="mt-4">
          <div className="relative h-3 w-full overflow-hidden rounded-full bg-neutral-800">
            ...
            <div
              ...
              title={`Expected: ${fmtCurrency(pacing.expectedSpend, currency)}`}
            />
          </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-3 border-t border-neutral-800 pt-4">
        <div>
          ...
          <p className="mt-0.5 text-sm font-medium tabular-nums text-neutral-200">
            {fmtCurrency(pacing.projectedSpend, currency)}
          </p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm font-medium tabular-nums text-neutral-200">
            {pacing.idealDailySpend ? fmtCurrency(pacing.idealDailySpend, currency) : "—"}
          </p>
        </div>
      </div>
    </div>
