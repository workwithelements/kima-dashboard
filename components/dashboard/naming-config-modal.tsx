...
export default function NamingConfigModal({ clientId, onClose, onSaved }: Props) {
  ...
  function addPosition() {
    const nextIndex = positions.length > 0
  ...
  // Add custom key
  function addCustomKey(idx: number) {
    if (!customKeyInput.trim()) return
    const key = customKeyInput.trim().replace(/\s+/g, "_").toLowerCase()
    updatePosition(idx, "key", key)
    updatePosition(idx, "label", customKeyInput.trim())
    setCustomKeyInput("")
  }
  ...
  // Get all unique dimension keys from positions
  ...
  return (
    ...
    >
      <div className="relative max-h-[85vh] w-full max-w-xl overflow-y-auto rounded-xl border border-neutral-700 bg-neutral-900 shadow-2xl">
        {/* Header */}
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-neutral-800 bg-neutral-900 px-5 py-4">
          ...
          <div className="space-y-6 p-5">
            ...
            <div>
              ...
              <p className="mb-3 text-[10px] text-neutral-500">
                Map each underscore position in your ad names to a dimension.
              </p>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <input
                        type="number"
                        value={pos.index}
                        onChange={(e) => updatePosition(idx, "index", parseInt(e.target.value) || 0)}
                        className="w-10 rounded border border-neutral-700 bg-neutral-900 px-1.5 py-0.5 text-xs text-white text-center focus:border-brand-lime focus:outline-none"
                        min={0}
                      />
                    </div>
                    ...
                    <select
                      value={STANDARD_KEYS.find((s) => s.key === pos.key) ? pos.key : "__custom__"}
                      onChange={(e) => {
                        if (e.target.value === "__custom__") {
                          // Keep current custom key
                        } else {
                    ...
                    {!STANDARD_KEYS.find((s) => s.key === pos.key) && (
                      <input
                        ...
                        value={pos.key}
                        onChange={(e) => {
                          const key = e.target.value.replace(/\s+/g, "_").toLowerCase()
                          updatePosition(idx, "key", key)
                        }}
                        placeholder="key"
                ...
                <div className="mt-3 space-y-4">
                  {usedKeys.length === 0 ? (
                    <p className="text-xs text-neutral-500">
                      Add positions above first, then configure value mappings per dimension.
                    </p>
                  ) : (
                    Array.from(new Set(usedKeys)).map((dimKey) => {
                      const dimLabel = positions.find((p) => p.key === dimKey)?.label || dimKey
                      const entries = Object.entries(valueMaps[dimKey] || {})
                      return (
                        <div key={dimKey} className="rounded-lg border border-neutral-800 p-3">
                          <p className="mb-2 text-xs font-medium text-neutral-300">
                            {dimLabel}
                          </p>
                          {entries.map(([code, label], eIdx) => (
                            <div key={eIdx} className="mb-1.5 flex items-center gap-2">
                              <input
                                type="text"
                                value={code}
                                onChange={(e) => updateValueMapEntry(dimKey, code, e.target.value, label)}
                                placeholder="Code"
                              ...
                              <input
                                type="text"
                                value={label}
                                onChange={(e) => updateValueMapEntry(dimKey, code, code, e.target.value)}
                                placeholder="Display label"
                                className="flex-1 rounded border border-neutral-700 bg-neutral-900 px-2 py-1 text-xs text-white focus:border-brand-lime focus:outline-none"
                              />
                              <button
            ...
            <div className="flex items-center justify-end gap-3 border-t border-neutral-800 pt-4">
              <button
                onClick={onClose}
                className="rounded-lg px-4 py-2 text-xs font-medium text-neutral-400 transition hover:text-white"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="rounded-lg bg-brand-lime px-4 py-2 text-xs font-semibold text-black transition hover:bg-brand-lime/90 disabled:opacity-50"
              >
                {saving ? "Saving…" : "Save"}
              </button>
            </div>
